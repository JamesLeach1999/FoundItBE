...
[Route("[controller]")]
[ApiController]
public class UsersController(IGetValues<User> userDb, ICreateValues<UserRequest, object> createUserValues, AbstractValidator<Guid> userIdValidator, AbstractValidator<UserRequest> userRequestValidator) : Controller
{
    ...
    [ProducesResponseType((int)HttpStatusCode.BadRequest)]
    [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
    public async Task<ActionResult<object>> CreateUser([FromBody] UserRequest user)
    {
        try
        {
